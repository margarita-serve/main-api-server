...
func (r *DeploymentRepo) Get(ID string) (*domEntity.Deployment, error) {
	// select db
	dbCon, err := r.handler.GetGormDB(r.dbConnectionName)
	if err != nil {
		return nil, err
	...
	var domEntity = &domEntity.Deployment{}
	var count int64
...
func (r *DeploymentRepo) ByName(name string) ([]domEntity.Deployment, error) {
	// select db
	print(name)
	dbCon, err := r.handler.GetGormDB(r.dbConnectionName)
	if err != nil {
	...
	var deployments []domEntity.Deployment
...
func (r *DeploymentRepo) GetAll(name string) ([]domEntity.Deployment, error) {
	// select db
	print(name)
	dbCon, err := r.handler.GetGormDB(r.dbConnectionName)
	if err != nil {
...
func (r *DeploymentRepo) Delete(id string) error {
	...
	var domEntity = &domEntity.Deployment{ID: id}
	var count int64

...
type CreateModelPackageRequestDTO struct {
	...
	Name                  string  `json:"name" example:"ModelPacakge 01" extensions:"x-order=01"`                                                              // 모델패키지 명
	Description           string  `json:"description" example:"New ModelPacakge" extensions:"x-order=02"`                                                      // 모델패키지 설명
	ModelName             string  `json:"modelName" example:"Example Model" extensions:"x-order=03"`                                                           // 모델명
	ModelVersion          string  `json:"modelVersion" example:"1.4" extensions:"x-order=04"`                                                                  // 모델 버전
	ModelDescription      string  `json:"modelDescription" example:"Best Accuracy" extensions:"x-order=05"`                                                    // 모델 설명
	TargetType            string  `json:"targetType" example:"Binary" extensions:"x-order=06" enums:"Binary, Regression"`                                      // 모델 예측타입 Binary(Binary classification), Regression
	PredictionTargetName  string  `json:"predictionTargetName" example:"Target" extensions:"x-order=07"`                                                       // 데이터 셋 예측타켓 컬럼 명칭
	ModelFrameWork        string  `json:"modelFrameWork" example:"TensorFlow" extensions:"x-order=08" enums:"TensorFlow, PyTorch, SkLearn, XGBoost, LightGBM"` // 모델 프레임워크
	ModelFrameWorkVersion string  `json:"modelFrameWorkVersion" example:"1.14.0" extensions:"x-order=09"`                                                      // 모델 프레임워크 버전

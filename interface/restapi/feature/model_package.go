...
func NewModelPackage(h *handler.Handler, projectSvc *appProjectSvc.ProjectService) (*FModelPackage, error) {
	var err error

	f := new(FModelPackage)
	f.handler = h

	if f.appModelPackage, err = appModelPackage.NewModelPackageApp(h, projectSvc); err != nil {
		return nil, err
	}
